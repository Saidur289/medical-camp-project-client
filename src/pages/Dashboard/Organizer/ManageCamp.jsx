  ...
  <table className="table">
    ...
    <tbody>
     ...
     {camps.length === 0? <><td>You have no data</td></>: camps.map((camp, index) =>  <tr key={index}>
        ...
        <td>{camp?.location}</td>
        <td>
            <Link to = {`/dashboard/updateCamp/${camp._id}`}><button className="btn btn-xs"><FaEdit className="text-myAccent"></FaEdit></button></Link>
            <button onClick={() => handleDelete(camp._id)} className="btn btn-xs"><FaTrash className="text-red-500"></FaTrash></button>
        </td>
      </tr>)}
