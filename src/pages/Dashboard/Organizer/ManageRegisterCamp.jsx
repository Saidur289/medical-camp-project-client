...
const ManageRegisterCamp = () => {
  ...
  const handleStatus = async(newStatus, id) => {
    ...
    try{
      await axiosSecure.patch(`/update-payment-status/${id}`, {status: newStatus})
              ...
              participants.map((participant, index) => (
                <tr key={index}>
                  ...
                  <td>
                    ...
                    <select onChange={(e) => handleStatus(e.target.value, participant._id)} disabled = {participant?.confirmStatus === 'Confirmed' || participant?.paymentStatus === 'Unpaid'} defaultValue={participant?.confirmStatus} className="select select-bordered select-xs w-full max-w-xs">
                      <option >
                        Update Status
                      </option>
                  ...
                  <td>
                    <button
                    onClick={() => handleDelete(participant._id, participant.campId)}
                      disabled={
